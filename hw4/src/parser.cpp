...
public:
	...
	enum ETag {
		...
		ETest                 = NoriObject::ETest,
                EEvaluator            = NoriObject::EEvaluator,
	...
	Vector3f parseVector(const QString &str) const {
        ...
        Eigen::Affine3f parseTransform(const QString &str) const {
                QRegExp re1("[\\s]*;[\\s]*");
		QStringList rows = str.split(re1);
		if (rows.size() != 4)
			throw NoriException(QString("Cannot parse affine transform '%1'!").arg(str));
		
                Eigen::Affine3f transform;
		for (int i=0; i<4; ++i){
                        QRegExp re2("[\\s,]+");
                        QStringList list = rows[i].split(re2);
                        if (list.size() != 4)
                                throw NoriException(QString("Cannot parse transform row '%1'!").arg(rows[i]));
                        for(int j=0; j < 4; ++j) transform(i, j) = parseFloat(list[j]);
                }
                return transform;
        }
	...
	bool endElement(const QString & /* unused */, const QString &name,
		...
		if (tag < NoriObject::EClassTypeCount) {
			...
			switch (tag) {
				...
				case ETransform: {
						QString direct = context.attr.value("value");
						if(direct.length() > 0) m_transform = parseTransform(direct); // TODO warning if we replace stuff!
...
NoriObject *loadScene(const QString &filename) {
	...
	if (!validator.validate(&file))
        ...
        NoriObjectFactory::setBasedir(QFileInfo(file).absoluteDir().path());
